...
func DecryptStrAes(pwd string, slt string, src string) (res string, err error) {
	...
	return string(removeByteZeroInArr(result)), nil
}

// genKey функция создаёт ключ кратный 16 байт, на основе пароля строки pwd
// дополняя его случайной последовательностью байт до длинны кратной 16 байт
// возвращает результат в массиве байт res и байты, которыми был дополнен ключ в массиве slt
...
// normalize16Block дополняет массив байт нолями до длинны кратной 16 байт
...
// removeByteZeroInArr удаляет нулевые байты в массиве и возвращает новый массив
func removeByteZeroInArr(src []byte) []byte {
	var result []byte
